...
namespace SimpleH2downloader { namespace AssetBundles {
    ...
    {
        WWWAssetBundle m_WWW;
        string m_Url;

        public AssetBundleDownloadFromWebOperation(string assetBundleName, WWWAssetBundle www)
            : base(assetBundleName)
        {
            if (www == null)
                throw new System.ArgumentNullException("www");
            m_Url = AssetBundleManager.BaseDownloadingURL + www.url;
            //Debug.Log("[AssetBundleDownloadFromWebOperation] url="+m_Url);
            this.m_WWW = www;
        }
        ...
        protected override void FinishDownload()
        {
            ...
            if (!string.IsNullOrEmpty(error)) {
                Debug.LogError("[AssetBundleDownloadFromWebOperation] error="+error);
                m_WWW.Dispose();
                m_WWW = null;
                return;
            }
            ...
            //Debug.Log("[AssetBundleDownloadFromWebOperation] finish");
            m_WWW.Dispose();
            m_WWW = null;
        }
    ...
    public class AssetBundleLoadLevelSimulationOperation : AssetBundleLoadOperation
    {
        ...
        public AssetBundleLoadLevelSimulationOperation(string assetBundleName, string levelName, bool isAdditive)
        {
            ...
            LoadSceneParameters lsp = new LoadSceneParameters(isAdditive ? LoadSceneMode.Additive : LoadSceneMode.Single);
            m_Operation = UnityEditor.SceneManagement.EditorSceneManager.LoadSceneAsyncInPlayMode(levelPaths[0], lsp);
#else
            if (isAdditive)
